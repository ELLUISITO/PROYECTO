...
let agregar_servicio = document.getElementById('agregar_servicio');
let contenido_servicio = document.getElementById('contenedor_servicio');


agregar_servicio.addEventListener('click', e =>{
    e.preventDefault();
    let clonado = document.querySelector('.clonar');
    let clon = clonado.cloneNode(true);

    contenido_servicio.appendChild(clon).classList.remove('clonar');

    let remover_ocutar = contenido_servicio.lastChild.childNodes[1].querySelectorAll('span');
    remover_ocutar[0].classList.remove('ocultar');
});

contenido_servicio.addEventListener('click', e =>{
    e.preventDefault();
    if(e.target.classList.contains('puntero')){
        let contenedor_servicio  = e.target.parentNode.parentNode;
    
        contenedor_servicio.parentNode.removeChild(contenedor_servicio);
    }
});